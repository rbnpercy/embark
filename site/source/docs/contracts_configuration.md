...
Interacting with Proxy contracts is usually difficult, because you have to point the Base contract to the address of the Proxy for it to work.
...
No need to import or use `BaseContract`, since `ContractInstance` contains both the ABI of `BaseContract` and `Proxy`.
